...
#if defined(NDEBUG)
# error "MBITBOOKS cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x5829ed19e288657f3f7f37d81ea35e93db626bd42b89b8e00d659e7b1f3bb82d");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // MBITBOOKS: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "MBITBOOKS Signed Message:\n";
...
{
    // MBITBOOKS: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // MBITBOOKS: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // MBITBOOKS: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1490220610, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "MBIT ";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 40 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04454b488beb392b4b8fd7de8b452cde63b2d941f640e39ba30ebce059781d27d4809dd9a65ee37543fc5bcd5180dff6d48f20774d41d4993a789274dd7af0c21c") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1490220610;
        ...
        assert(block.hashMerkleRoot == uint256("0x8160c4d0a124713f59d71dbec6bf38c85a2a349db6b795c30eaf0f83cf19c26a"));
...
unsigned char pchMessageStart[4] = { 0xbd, 0x29, 0xf3, 0x0a }; // MBITBOOKS: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MBITBOOKSMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MBITBOOKSMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MBITBOOKSMiner(CWallet *pwallet)
{
    printf("MBITBOOKSMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("mbitbooks-miner");
    ...
    try { loop {
        ...
        printf("Running MBITBOOKSMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MBITBOOKSMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MBITBOOKSMiner, pwallet));
}
